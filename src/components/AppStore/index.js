...
class AppStore extends Component {
  state = {userSearchInput: '', isActive: tabsList[0].tabId}
  ...
  getFilteredProjects = () => {
    ...
    const filteredProjects = appsList.filter(each => each.category === isActive)
  ...
  render() {
    ...
    const filter = filteredProjects.filter(each =>
      each.appName.toLowerCase().includes(userSearchInput.toLowerCase()),
    )
    return (
      <div className="bg-container">
        ...
        <ul className="unordered-tablist">
          {tabsList.map(each => (
            <TabItem
              clickTabItem={this.clickTabItem}
              key={each.tabId}
              tabListDetails={each}
              isActive={each.tabId === isActive}
            />
          ))}
        </ul>

        <ul className="unordered-Applist">
          {filter.map(each => (
            <AppItem key={each.appId} appListDetails={each} />
          ))}
        </ul>
      </div>
    )
